  ...
  try {
    ...
    try {
      await Hotel.findByIdAndUpdate(HotelId, {
        ...
        $pull: { rooms: req.params.id }, // here pull one unique room_id
      });
    } catch (errors) {
      next(errors);
    }
...
export const DeleteSpecificRoomOfSpecificHotel = async (req, res, next) => {
  ...
  try {
    for (let i = 0; i < length; i++) {
      if ((await Room.findById(RoomId)).rooms[i]._id === RoomNoId) {
        try {
          (await Room.findById(RoomId)).rooms[i].findByIdAndDelete(RoomNoId);
          // const DeletedSpecificRoomsOfSpecificHotel = (
          //   await Room.findById(RoomId)
          // ).rooms[i].findById(RoomNoId);
          res.status(200).json({
            status: 200,
            message:
              "Specific Room Of Rooms Record Deleted Successfully & Deleted Rooms Detils is Below ! 🔥🔥🔥",
            data: { DeletedSpecificRoomsOfSpecificHotel },
          });
          console.log("Room Record Deleted Successfully ! ⏰⏰⏰");
        } catch (error) {
          next(error);
